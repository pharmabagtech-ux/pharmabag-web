            ...
            <div className="bg-white/40 backdrop-blur-xl p-4 sm:p-6 md:p-8 rounded-[32px] sm:rounded-[40px] border border-white/60 shadow-xl mt-4">
              ...
              <div className="w-full pb-4">
                ...
                {listings.map((l: any, idx: number) => {
                  ...
                  // A scheme is bought in whole lots, so the stepper moves a lot
                  // at a time: a buy-9-get-1 goes 36 -> 45 -> 54, never 37.
                        ...
                        {listingInStock ? (
                          <div className="flex items-center justify-end w-full">
                            {(!listingCartItem || listingCartItem.quantity === 0) ? (
                              <button 
                                onClick={() => addToCart.mutate({ productId: l.id, quantity: minQty, productName: product.name, price: l.price, mrp: mrp, gstPercent: l.gstPercent ?? product.gstPercent, imageUrl: product.images?.[0], discountType: l.discountType, discountMeta: l.discountMeta })}
                              ...
                              <div className="flex items-center gap-1 sm:gap-2 w-full">
                                <div className="flex items-center bg-slate-900 rounded-lg sm:rounded-xl p-0.5 h-8 sm:h-12 shadow-sm w-full">
                                  ...
                                  <MarketplaceQtyInput 
                                    ...
                                    minQty={minQty}
                                    lot={lot}
                                    onUpdate={(nextQty) => addToCart.mutate({ productId: l.id, quantity: nextQty, productName: product.name, price: l.price, mrp: mrp, gstPercent: l.gstPercent ?? product.gstPercent, imageUrl: product.images?.[0], discountType: l.discountType, discountMeta: l.discountMeta, replace: true })}
...
function MarketplaceQtyInput({ 
  ...
  const handleBlur = () => {
    ...
    // Snap to an allowed quantity: at or above the minimum, in whole lots,
    // and within stock. Rounds down, so a typed figure never quietly costs
